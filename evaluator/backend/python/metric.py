...
def precision(rank: List[int], ground_truth: Set[int]) -> np.ndarray:
...
def recall(rank: List[int], ground_truth: Set[int]) -> np.ndarray:
...
def map(rank: List[int], ground_truth: Set[int]) -> np.ndarray:
    pre = precision(rank, ground_truth)
    pre = [pre[idx] if item in ground_truth else 0 for idx, item in
           enumerate(rank)]
    sum_pre = np.cumsum(pre, dtype=np.float32)
    # relevant_num = np.cumsum([1 if item in ground_truth else 0 for item in rank])
    relevant_num = np.cumsum(
        [min(idx + 1, len(ground_truth)) for idx, _ in enumerate(rank)])
    result: np.ndarray = [p / r_num if r_num != 0 else 0 for p, r_num in
                          zip(sum_pre, relevant_num)]
    return result


def ndcg(rank: List[int], ground_truth: Set[int]) -> np.ndarray:
    len_rank = len(rank)
    ...
    dcg = np.cumsum(
        [1.0 / np.log2(idx + 2) if item in ground_truth else 0.0 for idx, item
         in enumerate(rank)])
    result = dcg / idcg
    return result


def mrr(rank: List[int], ground_truth: Set[int]) -> np.ndarray:
