...
class Caser(SeqAbstractRecommender):
    ...
    def _create_variable(self):
        ...
        self.item_seq_ph = tf.placeholder(tf.int32, [None, self.seq_L],
                                          name="item_seq")
        self.item_pos_ph = tf.placeholder(tf.int32, [None, self.seq_T],
                                          name="item_pos")
        self.item_neg_ph = tf.placeholder(tf.int32, [None, self.neg_samples],
                                          name="item_neg")
        self.is_training = tf.placeholder(tf.bool, name="training_flag")

        l2_regularizer = tf.contrib.layers.l2_regularizer(self.l2_reg)

        self.user_embeddings = tf.get_variable('user_embeddings',
                                               dtype=tf.float32,
                                               shape=[self.users_num,
                                                      self.factors_num],
                                               regularizer=l2_regularizer)

        item_embeddings = tf.get_variable('seq_item_embeddings',
                                          dtype=tf.float32,
                                          shape=[self.items_num,
                                                 self.factors_num],
                                          regularizer=l2_regularizer)
        zero_pad = tf.zeros([1, self.factors_num], name="padding")
        self.seq_item_embeddings = tf.concat([item_embeddings, zero_pad],
                                             axis=0)

        self.conv_v = tf.layers.Conv2D(self.nv, [self.seq_L, 1])
        self.conv_h = [
            tf.layers.Conv2D(self.nh, [i, self.factors_num], activation="relu")
        ...
        self.item_embeddings = tf.get_variable('item_embeddings',
                                               ...
                                               shape=[self.items_num,
                                                      self.factors_num * 2],
                                               regularizer=l2_regularizer)
        self.item_biases = tf.get_variable('item_biases', dtype=tf.float32,
                                           shape=[self.items_num],
    ...
    def build_graph(self):
        ...
        item_embs = tf.nn.embedding_lookup(self.seq_item_embeddings,
                                           self.item_seq_ph)  # (b, L, d)
        item_embs = tf.expand_dims(item_embs, axis=3)  # (b, L, d, 1)

        # vertical conv layer
        out_v = self.conv_v(item_embs)  # (b, 1, d, nv)
        out_v = tf.reshape(out_v, shape=[batch_size,
                                         self.nv * self.factors_num])  # (b, nv*d)
        ...
        # rating calculation
        user_embs = tf.nn.embedding_lookup(self.user_embeddings,
                                           self.user_ph)  # (b, d)
        user_embs = tf.concat([z, user_embs], axis=1)  # (b, 2d)
        user_embs = tf.expand_dims(user_embs, axis=1)  # (b, 1, 2d)

        tar_item = tf.concat([self.item_pos_ph, self.item_neg_ph],
                             axis=-1)  # (b, 2T)
        tar_item_embs = tf.nn.embedding_lookup(self.item_embeddings,
                                               tar_item)  # (b, 2T, 2d)
        tar_item_bias = tf.gather(self.item_biases, tar_item)  # (b, 2T)
        logits = tf.squeeze(
            tf.matmul(user_embs, tar_item_embs, transpose_b=True),
            axis=1) + tar_item_bias  # (b, 2T)
        pos_logits, neg_logits = tf.split(logits,
                                          [self.seq_T, self.neg_samples],
                                          axis=1)
        ...
        self.train_opt = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(
            loss)

        # for predict/test
        user_embs = tf.squeeze(user_embs, axis=1)  # (b, 2d)
        self.all_logits = tf.matmul(user_embs, self.item_embeddings,
                                    transpose_b=True)  # (b, items_num)
    ...
    def _generate_sequences(self):
        ...
        for user in uni_users:
            ...
            if len(seq_items) - seq_len >= 0:
                ...
                seq_items = np.reshape(seq_items, newshape=[1, -1]).astype(
                    np.int32)
                seq_items = pad_sequences(seq_items, value=self.items_num,
                                          max_len=seq_len,
    ...
    def _sample_negative(self, users_list):
        ...
        user_count = DataIterator(all_uni_user, all_counts, batch_size=1024,
                                  shuffle=False)
