...
class Fossil(SeqAbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.name_scope("input_data"):
            self.user_input_id = tf.placeholder(tf.int32, shape=[None, ],
                                                name="user_input_id")  # the index of users
            self.user_input = tf.placeholder(tf.int32, shape=[None, None],
                                             name="user_input")  # the index of users
            ...
            self.item_input = tf.placeholder(tf.int32, shape=[None, ],
                                             name="item_input_pos")  # the index of items
            self.item_input_recent = tf.placeholder(tf.int32,
                                                    shape=[None, None],
                                                    name="item_input_recent")
            if self.is_pairwise is True:
                self.user_input_neg = tf.placeholder(tf.int32,
                                                     shape=[None, None],
                                                     name="user_input_neg")
                self.item_input_neg = tf.placeholder(tf.int32, shape=[None, ],
                                                     name="item_input_neg")
                self.num_idx_neg = tf.placeholder(tf.float32, shape=[None, ],
                                                  name="num_idx_neg")
            else:
    ...
    def _create_variables(self):
        with tf.name_scope(
                "embedding"):  # The embedding initialization is unknown now
            initializer = tool.get_initializer(self.init_method, self.stddev)

            self.c1 = tf.Variable(
                initializer([self.num_items, self.embedding_size]), name='c1',
                dtype=tf.float32)
            self.c2 = tf.constant(0.0, tf.float32, [1, self.embedding_size],
                                  name='c2')
            self.embedding_P = tf.concat([self.c1, self.c2], 0,
                                         name='embedding_P')
            self.embedding_Q = tf.Variable(
                initializer([self.num_items, self.embedding_size]),
                name='embedding_Q', dtype=tf.float32)

            self.eta = tf.Variable(
                initializer([self.num_users, self.high_order]), name='eta')
            self.eta_bias = tf.Variable(initializer([1, self.high_order]),
                                        name='eta_bias')
    ...
    def _create_inference(self, user_input, item_input, num_idx):
        with tf.name_scope("inference"):
            embedding_p = tf.reduce_sum(
                tf.nn.embedding_lookup(self.embedding_P, user_input), 1)
            embedding_eta_u = tf.nn.embedding_lookup(self.eta,
                                                     self.user_input_id)
            batch_size = tf.shape(embedding_eta_u)[0]
            eta = tf.expand_dims(tf.tile(self.eta_bias, tf.stack(
                [batch_size, 1])) + embedding_eta_u, -1)
            embeddings_short = tf.nn.embedding_lookup(self.embedding_P,
            ...
            output = coeff * tf.reduce_sum(
                tf.multiply(embedding_p, embedding_q), 1) + \
                     tf.reduce_sum(
                         tf.multiply(tf.reduce_sum(eta * embeddings_short, 1),
                                     embedding_q), 1) + bias_i
        return embedding_p, embedding_q, embedding_eta_u, embeddings_short, output

    def _create_loss(self):
        with tf.name_scope("loss"):
            p1, q1, eta_u, short, self.output = self._create_inference(
                self.user_input, self.item_input, self.num_idx)
            if self.is_pairwise is True:
                _, q2, _, _, output_neg = self._create_inference(
                    self.user_input_neg, self.item_input_neg, self.num_idx_neg)
                self.result = self.output - output_neg
                self.loss = learner.pairwise_loss(self.loss_function,
                                                  self.result) + \
    ...
    def _create_optimizer(self):
        with tf.name_scope("learner"):
            self.optimizer = learner.optimizer(self.learner, self.loss,
                                               self.learning_rate)
        ...
        for epoch in range(1, self.num_epochs + 1):
            if self.is_pairwise is True:
                ...
                num_idx_neg, item_input_pos, item_input_neg, item_input_recent = \
                    data_generator._get_pairwise_all_likefossil_data(
                        self.dataset, self.high_order, self.train_dict)

                data_iter = DataIterator(user_input_id, user_input,
                                         user_input_neg, num_idx_pos,
                                         num_idx_neg,
                                         item_input_pos, item_input_neg,
                                         item_input_recent,
                                         batch_size=self.batch_size,
                                         shuffle=True)
            else:
                user_input_id, user_input, num_idx, item_input, item_input_recent, labels = \
                    data_generator._get_pointwise_all_likefossil_data(
                        self.dataset, self.high_order,
                        self.num_negatives, self.train_dict)

                data_iter = DataIterator(user_input_id, user_input, num_idx,
                                         item_input, item_input_recent, labels,
                                         batch_size=self.batch_size,
                                         shuffle=True)
        ...
        if candidate_items_userids is None:
            ...
            for user_id in user_ids:
                ...
                for _ in range(self.num_items):
                    item_recent.append(items_by_user_id[len(
                        items_by_user_id) - self.high_order:])
            ...
            for user_id, eval_items_by_user_id in zip(user_ids,
                                                      candidate_items_userids):
                items_by_user_id = self.train_dict[user_id]
                num_idx = len(items_by_user_id)
                # Get prediction scores
                item_idx = np.full(len(eval_items_by_user_id), num_idx,
                                   dtype=np.int32)
                user_input = []
                user_input.extend(
                    [items_by_user_id] * len(eval_items_by_user_id))
                item_recent = []
                for _ in range(len(eval_items_by_user_id)):
                    item_recent.append(items_by_user_id[len(
                        items_by_user_id) - self.high_order:])
                users = np.full(len(eval_items_by_user_id), user_id,
                                dtype=np.int32)
                feed_dict = {self.user_input_id: users,
                             self.user_input: user_input,
                             self.num_idx: item_idx,
