    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            ...
            embeddings_IU_i = tf.nn.embedding_lookup(self.embeddings_IU,
                                                     self.item_input)
            embeddings_LI_l = tf.nn.embedding_lookup(self.embeddings_IL,
                                                     self.item_input_recent)

            context_embedding = tf.reduce_sum(embeddings_LI_l, 1)
            predict_vector = tf.multiply(tf.nn.relu(embeddings_UI_u),
                                         tf.nn.relu(embeddings_IU_i)) + \
                             tf.multiply(tf.nn.relu(embeddings_IU_i),
                                         tf.nn.relu(context_embedding))
