...
class TransRec(SeqAbstractRecommender):
    def __init__(self, conf):
        super(TransRec, self).__init__(conf)
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            self.item_biases = tf.Variable(initializer([self.num_items]),
                                           name='item_biases',
                                           dtype=tf.float32)  # (items)
            self.global_embedding = tf.Variable(
                initializer([1, self.embedding_size]),
                name='global_embedding', dtype=tf.float32)

    def _create_inference(self, item_input):
        with tf.name_scope("inference"):
            # embedding look up
            user_embedding = tf.nn.embedding_lookup(self.user_embeddings,
                                                    self.user_input)
            batch_size = tf.shape(user_embedding)[0]
            item_embedding_recent = tf.nn.embedding_lookup(self.item_embeddings,
                                                           self.item_input_recent)
            item_embedding = tf.nn.embedding_lookup(self.item_embeddings,
                                                    item_input)

            item_bias = tf.nn.embedding_lookup(self.item_biases, item_input)
            predict_vector = user_embedding + tf.tile(self.global_embedding,
                                                      tf.stack(
                                                          [batch_size, 1])) + \
    ...
    def build_graph(self):
        ...
        last_i_emb = tf.nn.embedding_lookup(self.item_embeddings,
                                            self.item_input_recent)
