...
class SRGNN(SeqAbstractRecommender):
    ...
    def _create_variable(self):
        ...
        self.alias_ph = tf.placeholder(dtype=tf.int32, shape=[self.batch_size,
                                                              None])  # 给给每个输入重新
        self.item_ph = tf.placeholder(dtype=tf.int32, shape=[self.batch_size,
                                                             None])  # 原始ID+padID, pad 在后面
        self.target_ph = tf.placeholder(dtype=tf.int32, shape=[self.batch_size])

        self.adj_in_ph = tf.placeholder(dtype=tf.float32,
                                        shape=[self.batch_size, None, None])
        self.adj_out_ph = tf.placeholder(dtype=tf.float32,
                                         shape=[self.batch_size, None, None])

        stdv = 1.0 / math.sqrt(self.hidden_size)
        w_init = tf.random_uniform_initializer(-stdv, stdv)
        self.nasr_w1 = tf.get_variable('nasr_w1',
                                       [self.hidden_size, self.hidden_size],
                                       dtype=tf.float32,
                                       initializer=w_init)
        self.nasr_w2 = tf.get_variable('nasr_w2',
                                       [self.hidden_size, self.hidden_size],
                                       dtype=tf.float32,
                                       initializer=w_init)
        self.nasr_v = tf.get_variable('nasrv', [1, self.hidden_size],
                                      dtype=tf.float32, initializer=w_init)
        self.nasr_b = tf.get_variable('nasr_b', [self.hidden_size],
                                      dtype=tf.float32,
        ...
        self.W_in = tf.get_variable('W_in',
                                    ...
                                    dtype=tf.float32,
                                    initializer=w_init)
        self.b_in = tf.get_variable('b_in', [self.hidden_size],
                                    dtype=tf.float32, initializer=w_init)
        self.W_out = tf.get_variable('W_out',
                                     [self.hidden_size, self.hidden_size],
                                     dtype=tf.float32,
                                     initializer=w_init)
        self.b_out = tf.get_variable('b_out', [self.hidden_size],
                                     dtype=tf.float32, initializer=w_init)
    ...
    def ggnn(self):
        fin_state = tf.nn.embedding_lookup(self.embedding,
                                           self.item_ph)  # (b,l,d)
        cell = tf.nn.rnn_cell.GRUCell(self.hidden_size)
        with tf.variable_scope('gru'):
            for i in range(self.step):
                fin_state = tf.reshape(fin_state, [self.batch_size, -1,
                                                   self.hidden_size])  # (b,l,d)
                fin_state_tmp = tf.reshape(fin_state,
                                           [-1, self.hidden_size])  # (b*l,d)

                fin_state_in = tf.reshape(
                    tf.matmul(fin_state_tmp, self.W_in) + self.b_in,
                ...
                fin_state_out = tf.reshape(
                    tf.matmul(fin_state_tmp, self.W_out) + self.b_out,
                    [self.batch_size, -1, self.hidden_size])  # (b,l,d)
                ...
                av = tf.expand_dims(tf.reshape(av, [-1, 2 * self.hidden_size]),
                                    axis=1)  # (b*l,1,2d)
                # fin_state_tmp = tf.reshape(fin_state, [-1, self.hidden_size])  # (b*l,d)

                state_output, fin_state = tf.nn.dynamic_rnn(cell, av,
                                                            initial_state=fin_state_tmp)
        return tf.reshape(fin_state,
                          [self.batch_size, -1, self.hidden_size])  # (b,l,d)

    def _session_embedding(self, re_embedding):
        # re_embedding  (b,l,d)
        rm = tf.reduce_sum(self.mask_ph, 1)  # (b,), length of each session
        last_idx = tf.stack([tf.range(self.batch_size), tf.to_int32(rm) - 1],
                            axis=1)  # (b, 2) index of last item
        last_id = tf.gather_nd(self.alias_ph,
                               last_idx)  # (b,) alias id of last item
        last_h = tf.gather_nd(re_embedding,
                              tf.stack([tf.range(self.batch_size), last_id],
                                       axis=1))  # (b,d) embedding of last item

        seq_h = [tf.nn.embedding_lookup(re_embedding[i], self.alias_ph[i]) for i
        ...
        m = tf.nn.sigmoid(last + tf.reshape(seq, [self.batch_size, -1,
                                                  self.hidden_size]) + self.nasr_b)
        coef = tf.matmul(tf.reshape(m, [-1, self.hidden_size]), self.nasr_v,
                         transpose_b=True) * tf.reshape(self.mask_ph, [-1, 1])
        if not self.nonhybrid:
            ma = tf.concat([tf.reduce_sum(
                tf.reshape(coef, [self.batch_size, -1, 1]) * seq_h, 1),
                tf.reshape(last, [-1, self.hidden_size])], -1)
            sess_embedding = tf.matmul(ma, self.B)
        else:
            sess_embedding = tf.reduce_sum(
                tf.reshape(coef, [self.batch_size, -1, 1]) * seq_h, 1)
    ...
    def build_graph(self):
        ...
        self.all_logits = tf.matmul(sess_embedding, item_embedding,
                                    transpose_b=True)
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=self.target_ph, logits=self.all_logits)
        loss = tf.reduce_mean(loss)

        vars = tf.trainable_variables()
        lossL2 = [tf.nn.l2_loss(v) for v in vars if
                  v.name not in ['bias', 'gamma', 'b', 'g', 'beta']]
        ...
        learning_rate = tf.train.exponential_decay(self.lr,
                                                   ...
                                                   decay_steps=decay,
                                                   decay_rate=self.lr_dc,
                                                   staircase=True)
        self.train_opt = tf.train.AdamOptimizer(learning_rate).minimize(
            loss_train, global_step=global_step)

    def train_model(self):
        train_seq_len = [(idx, len(seq)) for idx, seq in
                         enumerate(self.train_seq)]
    ...
    def _shuffle_index(self, seq_index):
        ...
        index_chunks_iter = DataIterator(index_chunks, batch_size=1,
                                         shuffle=True,
                                         drop_last=False)  # shuffle index chunk
        for indexes in index_chunks_iter:
            indexes = indexes[0]
            indexes_iter = DataIterator(indexes, batch_size=self.batch_size,
                                        shuffle=True,
    ...
    def predict(self, users, items):
        users = DataIterator(users, batch_size=self.batch_size, shuffle=False,
                             drop_last=False)
        all_ratings = []
        for bat_user in users:
            cur_batch_size = len(bat_user)
            bat_items = [self.user_pos_train[user][-self.max_seq_len:] for user
                         in bat_user]
            bat_adj_in, bat_adj_out, bat_alias, bat_items, bat_mask = self._build_session_graph(
                bat_items)
            if cur_batch_size < self.batch_size:  # padding
                ...
                bat_mask = np.concatenate([bat_mask, [bat_mask[-1]] * pad_size],
                                          axis=0)
        ...
        if items is not None:
            all_ratings = [all_ratings[idx][u_item] for idx, u_item in
                           enumerate(items)]
