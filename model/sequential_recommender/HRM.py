        ...
        with tf.name_scope("inference"):
            ...
            if self.high_order > 1:
                if self.session_agg == "max":
                    item_embedding_short = self.max_pooling(
                        item_embedding_recent)
                else:
                    item_embedding_short = self.avg_pooling(
                        item_embedding_recent)
                concat_user_item = tf.concat([tf.expand_dims(user_embedding, 1),
                                              tf.expand_dims(
                                                  item_embedding_short, 1)],
                                             axis=1)
            else:
                concat_user_item = tf.concat([tf.expand_dims(user_embedding, 1),
                                              tf.expand_dims(
                                                  item_embedding_recent, 1)],
                                             axis=1)
            if self.pre_agg == "max":
    ...
    def predict(self,
        ...
        if candidate_items_user_ids is None:
            ...
            for user_id in user_ids:
                ...
                # TODO skip if cand_items is empty?
                item_recent = []
