...
class FPMC(SeqAbstractRecommender):
    ...
    def _create_placeholders(self):
        with tf.name_scope("input_data"):
            ...
            self.item_input = tf.placeholder(tf.int32, shape=[None],
                                             name="item_input_pos")
            self.item_input_recent = tf.placeholder(tf.int32, shape=[None],
    ...
    def _create_inference(self, item_input):
        with tf.name_scope("inference"):
            ...
            predict_vector = tf.multiply(embeddings_UI_u,
                                         embeddings_IU_i) + tf.multiply(
                embeddings_IL_i, embeddings_LI_l)
    ...
    def train_model(self):
        ...
        if self.is_pairwise is True:
            data_iter = TimeOrderPairwiseSampler(self.dataset, high_order=1,
                                                 neg_num=1,
    ...
    def predict(self, user_ids, candidate_items_userids=None):
        ...
        if candidate_items_userids is None:
            ...
            for user_id in user_ids:
                ...
                item_recent = np.full(self.num_items, cand_items[-1],
                                      dtype=np.int32)
            ...
            for user_id, items_by_user_id in zip(user_ids,
                ...
                cand_items = self.train_dict[user_id]
                item_recent = np.full(len(items_by_user_id), cand_items[-1],
                                      dtype=np.int32)
