...
class WRMF(AbstractRecommender):
    def __init__(self, conf):
        ...
        self.Cui = np.zeros(shape=[self.num_users, self.num_items],
                            dtype=np.float32)
        self.Pui = np.zeros(shape=[self.num_users, self.num_items],
                            dtype=np.float32)
    ...
    def _create_variables(self):
        ...
        self.user_embeddings = tf.Variable(
            ...
            name='user_embeddings', dtype=tf.float32)
        self.item_embeddings = tf.Variable(
            initializer([self.num_items, self.embedding_size]),
            name='item_embeddings', dtype=tf.float32)

    def _create_optimizer(self):
        YTY = tf.matmul(self.item_embeddings, self.item_embeddings,
                        transpose_a=True)
        YTCuIY = tf.matmul(self.item_embeddings,
                           tf.multiply(self.Cu, self.item_embeddings),
                           transpose_a=True)
        YTCupu = tf.matmul(self.item_embeddings,
                           tf.multiply(self.Cu + 1, self.Pu), transpose_a=True)
        xu = tf.linalg.solve(YTY + YTCuIY + self.lambda_eye, YTCupu)
        self.update_user = tf.scatter_update(self.user_embeddings, self.user_id,
                                             tf.transpose(xu))

        XTX = tf.matmul(self.user_embeddings, self.user_embeddings,
                        transpose_a=True)
        XTCIIX = tf.matmul(self.user_embeddings,
                           tf.multiply(self.Ci, self.user_embeddings),
                           transpose_a=True)
        XTCIpi = tf.matmul(self.user_embeddings,
                           tf.multiply(self.Ci + 1, self.Pi), transpose_a=True)
        xi = tf.linalg.solve(XTX + XTCIIX + self.lambda_eye, XTCIpi)
        self.update_item = tf.scatter_update(self.item_embeddings, self.item_id,
                                             tf.transpose(xi))
