...
def _get_pairwise_all_likefism_data(dataset):
    ...
    for u in range(num_users):
        ...
        if num_items_by_u > 1:
            negative_items = randint_choice(num_items, num_items_by_u,
                                            replace=True, exclusion=items_by_u)
...
def _get_pairwise_all_likefossil_data(dataset, high_order, train_dict):
    ...
    for u in range(dataset.num_users):
        ...
        if num_items_by_u > high_order:
            negative_items = randint_choice(dataset.num_items, num_items_by_u,
                                            replace=True,
...
def _get_pointwise_all_likefossil_data(dataset, high_order, num_negatives,
                                       train_dict):
